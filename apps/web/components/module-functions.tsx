...
export function ForumExperience({contenido,modoVistaPrevia=false,onPublish}:{contenido:string;modoVistaPrevia?:boolean;onPublish:()=>void}){
  ...
  const minimo=40;
  const limite=1200;
  const caracteres=respuesta.trim().length;
  function publicar(){
    if(caracteres<minimo)return;
    onPublish();
    setRespuesta("");
  }
  return <div className="forum-experience">
    <header className="forum-heading">
      <span className="forum-icon"><MessageSquareText size={24}/></span>
      <div><span className="forum-eyebrow">FORO DE DISCUSIÓN</span><h3>Participa en la conversación</h3><p>Comparte una idea propia y construye sobre los aportes de tus compañeros.</p></div>
      <span className="forum-status">Foro abierto</span>
    </header>
    <section className="forum-prompt">
      <div className="forum-prompt-label"><MessageSquareText size={16}/><strong>Pregunta de discusión</strong></div>
      <p>{contenido}</p>
    </section>
    <div className="forum-guidelines">
      <span><CheckCircle2 size={15}/>Sustenta tu opinión</span>
      <span><CheckCircle2 size={15}/>Mantén un tono respetuoso</span>
      <span><CheckCircle2 size={15}/>Evita compartir datos personales</span>
    </div>
    <section className="forum-composer">
      <header><div><h4>Tu aporte</h4><p>{modoVistaPrevia?"Prueba cómo se verá la participación del estudiante.":"Escribe una respuesta clara y relacionada con la pregunta."}</p></div><span className={caracteres>=minimo?"is-ready":""}>{respuesta.length}/{limite}</span></header>
      <textarea value={respuesta} onChange={(event)=>setRespuesta(event.target.value)} maxLength={limite} aria-label={modoVistaPrevia?"Respuesta de vista previa":"Respuesta del estudiante"} placeholder="Escribe tu aporte aquí…"/>
      <footer><small>{caracteres<minimo?`Escribe al menos ${minimo-caracteres} caracteres más.`:"Tu aporte está listo para publicarse."}</small><button disabled={caracteres<minimo} onClick={publicar}><Send size={15}/>Publicar aporte</button></footer>
    </section>
    <section className="forum-thread-empty" aria-label="Respuestas del foro">
      <span className="forum-empty-icon"><MessageSquareText size={20}/></span>
      <div><strong>Aún no hay respuestas visibles</strong><p>Sé el primero en iniciar una conversación útil para el grupo.</p></div>
      <span>0 aportes</span>
    </section>
  </div>;
}
...
export function LmsBuilder(){
  ...
  async function persistir(next:ModuloLms[],success:string){if(!cursoId)return false;setGuardando(true);setError("");setMensaje("");try{const endpoint=`/academico/courses/${encodeURIComponent(cursoId)}/learning-path`;const result=await api<RutaLms>(endpoint,{method:"PUT",body:JSON.stringify({titulo:ruta.titulo||cursos.find((item)=>item.id===cursoId)?.nombre||"Ruta de aprendizaje",descripcion:ruta.descripcion||"Ruta de aprendizaje del curso.",estado:ruta.estado,modulos:next})});const confirmed=await api<RutaLms>(endpoint,{cache:"no-store"});if(confirmed.version<result.version||JSON.stringify(confirmed.modulos)!==JSON.stringify(result.modulos))throw new Error("Firebase recibió la edición, pero la lectura de verificación no coincide. Intenta nuevamente.");const{modulos:items,...metadata}=confirmed;setRuta(metadata);setModulos(items);setMensaje(`${success} Verificado en Firebase (versión ${confirmed.version}).`);return true;}catch(reason){setError(reason instanceof Error?reason.message:"No fue posible guardar los cambios");return false;}finally{setGuardando(false);}}
