...
const modulosIniciales:ModuloLms[]=[
  {id:"mod-1",titulo:"Módulo 1 · Fundamentos",descripcion:"Conceptos esenciales y contexto del curso.",estado:"Publicado",elementos:[
    {id:"elm-11",tipo:"PASO",titulo:"1.1 Bienvenida y objetivos",descripcion:"Presentación del curso y resultados esperados.",contenidoTipo:"Video",contenido:"Video introductorio del curso.",tiempo:8,requisito:"Libre",estado:"Publicado"},
    {id:"elm-12",tipo:"PASO",titulo:"1.2 Lectura: conceptos base",descripcion:"Lectura guiada para iniciar el curso.",contenidoTipo:"HTML",contenido:"Conceptos fundamentales y ejemplos.",tiempo:15,requisito:"Completar paso anterior",estado:"Publicado"},
    {id:"elm-13",tipo:"FORO",titulo:"Foro de presentación",descripcion:"Presenta tu experiencia y expectativas.",contenidoTipo:"Texto",contenido:"Comparte una breve presentación.",tiempo:10,requisito:"Libre",estado:"Abierto"},
    {id:"elm-14",tipo:"EVALUACION",titulo:"Evaluación diagnóstica",descripcion:"Cuestionario inicial de conocimientos.",contenidoTipo:"Cuestionario",contenido:"10 preguntas de opción múltiple.",tiempo:20,requisito:"Completar módulo",estado:"Publicada"},
  ]},
  {id:"mod-2",titulo:"Módulo 2 · Preparación de datos",descripcion:"Carga, limpieza y validación.",estado:"Borrador",elementos:[
    {id:"elm-21",tipo:"PASO",titulo:"2.1 Fuentes de información",descripcion:"Identificación de fuentes confiables.",contenidoTipo:"Texto",contenido:"Guía de fuentes y criterios.",tiempo:12,requisito:"Completar módulo anterior",estado:"Publicado"},
    {id:"elm-22",tipo:"PASO",titulo:"2.2 Laboratorio de limpieza",descripcion:"Práctica aplicada.",contenidoTipo:"Archivo",contenido:"Plantilla del laboratorio.",tiempo:35,requisito:"Completar paso anterior",estado:"Borrador"},
    {id:"elm-23",tipo:"FORO",titulo:"Foro: calidad de datos",descripcion:"Discusión de casos.",contenidoTipo:"Texto",contenido:"Analiza un problema de calidad.",tiempo:15,requisito:"Libre",estado:"Programado"},
  ]},
];
...
export function LmsBuilder(){
  ...
  function moverModulo(indice:number,direccion:-1|1){const destino=indice+direccion;if(destino<0||destino>=modulos.length)return;setModulos((actuales)=>{const copia=[...actuales];[copia[indice],copia[destino]]=[copia[destino]!,copia[indice]!];return copia;});setMensaje("Orden de módulos actualizado.");}
  function eliminarModulo(indice:number){const titulo=modulos[indice]?.titulo;setModulos((actuales)=>actuales.filter((_,i)=>i!==indice));setMensaje(`${titulo} eliminado.`);}
...
export function StudentLearningPath(){
  const[active,setActive]=useState(0);const[message,setMessage]=useState("");
  const contents=modulosIniciales.flatMap((modulo,moduleIndex)=>modulo.elementos.filter((elemento)=>elemento.estado!=="Borrador"&&elemento.estado!=="Programado").map((elemento)=>({modulo,moduleIndex,elemento})));
